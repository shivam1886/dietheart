...
@section('title') {{__('Food List')}} @endsection

@section('content')
<div class="row">
    <div class="col-lg-12">
        <h3 class="page-header">{{__('Foods')}} ({{$foods->total()}})</h3>
    </div>
</div>
...
<div class="row">
    <div class="col-lg-12 text-right">
        <a href="{{route('create.food')}}" class="btn btn-primary">{{__('Add food')}}</a>
    </div>
</div>
<br/>
<div class="row">
    <div class="col-lg-12">
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                ...
                <tbody>
                    @php $i = $foods->perPage() * ($foods->currentPage() - 1); @endphp
                    @foreach ($foods as $key => $value)
                        <tr class="odd gradeX">
                            <td>{{++$i}}</td>
                            <td>
                                    <div class="pictures-viewers">
                                         <img data-original="{{$value->image}}" src="{{$value->image}}" width="75px" height="50px" alt="{{$value->food}}">
                                    </div>
                            </td>
                            <td>{{$value->title ?? '-'}}</td>
                            <td>{{$value->cholesterol}} (mg)</td>
                            <td class="center">{{$value->is_active == '1' ? __('Active') : __('Deactive') }}</td>
                            <td>
                                <a href="{{route('show.food',$value->id)}}" class="btn btn-sm btn-primary">{{__('Info')}}</a>
